...
const HomeRightBox = () => {
    ...
    const now = new Date()
    const event = new Date("2021", "04", "15")

    let CurrentTime = now.getTime()
    let eventTime = event.getTime()

    let TimeForEvent = eventTime - CurrentTime

    let seconds = Math.floor( TimeForEvent/ 1000)
      ...
      <div className="RightBox">
        <div className="GroupMembers">
          <div className="PeopleBox">
            <h4>{t("Home.membersSup")}</h4>
            <h3>105</h3>
            <h5>{t("Home.membersInf")}</h5>
          </div>
          <div className="PeopleImg">
            <img alt="MembersAmountSVG" src={People}></img>
          </div>
        </div>
